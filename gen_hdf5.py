...
if __name__ == "__main__":
    ...
    record_path = os.path.join(path, f'{mpid}_record{nx}{ny}{nz}.txt')
    record = []
    ...
    for i in range(nx):
        for j in range(ny):
            for k in range(nz):
                ...
                nxnynz = str(i)+" "+str(j)+" "+str(k)+" "
                print(nxnynz)
                os.system('rm ')
                os.system(f"cd {path}; phono3py --abinit --dim=\""+str(nx_sc)+" "+str(ny_sc)+" "+str(nz_sc)+"\" -c pc.in --mesh=\""+str(nx)+" "+str(ny)+" "+str(nz)+"\" --sym-fc --fc3 --fc2 --br --write-phonon --write-gamma --loglevel=2 --ga=\""+nxnynz+"\"") 
                record.append(nxnynz + '   \n')		#!!
                with open(record_path, 'w') as ff:
                    for r in record:
                        ff.write(r)
    print(temp)
